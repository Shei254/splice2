...
class ToyyibpayController extends Controller
{
    ...
    public function charge(Request $request)
    {
        try {
            ...
            {
                $coupon_id = null;
                // $price = (float)$plan->{$request->toyyibpay_payment_frequency . '_price'};
                ...
                if(!empty($request->coupon))
                {
                    ...
                    if(!empty($coupons))
                    {
                        ...
                        // $discount_value = ( $price = (float)$plan->{$request->toyyibpay_payment_frequency . '_price'} / 100) * $coupons->discount;
                        // $price          = $price = (float)$plan->{$request->toyyibpay_payment_frequency . '_price'} - $discount_value;
                ...
                $this->callBackUrl = route('plan.status', [$plan->id, $price, $coupon]);
                $this->returnUrl = route('plan.status', [$plan->id, $price, $coupon]);
                $Date = date('d-m-Y');
                $ammount = $price;
