...
@section('content')
    @can('edit-tickets')
        ...
        @foreach ($attachments as $index => $attachment)
            <form method="post" id="user-form-{{ $index }}" action="{{ route('tickets.attachment.destroy', [$ticket->id, $index]) }}">
                @csrf
                @method('DELETE')
            </form>
        @endforeach
                            ...
                            <form method="post" action="{{ route('note.store', $ticket->id) }}">
                                ...
                                <div class="card-body adjust_card_width">
                                    <div class="form-group ckfix_height">
                                        <textarea name="note" id="note" class="form-control ckdescription">{{ $ticket->note }}</textarea>
                                        <div class="invalid-feedback">
                                            {{ $errors->first('note') }}
                                        </div>
                                    </div>
